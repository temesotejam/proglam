...
WaypointApplyResult applyWaypointSet(WaypointSafetyState safety, const WaypointRequest& request, WaypointStore& store) {
  ...
  if (request.action != 1 || request.count > 16 || !std::isfinite(request.reachRadiusM) || request.reachRadiusM <= 0 || (request.count && request.points == nullptr)) { result.reason=request.count==0 ? WaypointApplyReason::Empty : WaypointApplyReason::Range; return result; }
