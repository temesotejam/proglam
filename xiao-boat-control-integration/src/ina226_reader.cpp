...
bool Ina226::read16(uint8_t r,uint16_t&v){Wire.beginTransmission(kInaAddress);Wire.write(r);if(Wire.endTransmission(false)!=0)return false;if(Wire.requestFrom((int)kInaAddress,2)!=2)return false;v=((uint16_t)Wire.read()<<8)|Wire.read();return true;}
bool Ina226::write16(uint8_t r,uint16_t v){Wire.beginTransmission(kInaAddress);Wire.write(r);Wire.write(v>>8);Wire.write(v);return Wire.endTransmission()==0;}
