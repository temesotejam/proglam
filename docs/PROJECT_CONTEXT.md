...
- BNO08Xは専用I2C 100 kHzを維持する。D3 INTのISRはI2Cを実行せず、専用タスクへの通知だけを行う。通信・記録側はSH-2コールバックで復号済みの全イベントをキュー化してからログへ渡す。BOAT23では加速度・較正ジャイロ各100 Hz、較正地磁気20 Hzを要求し、Game Rotation Vectorは要求しない。RUN0011後、制御側BNOにもSH-2コールバック＋96件イベントキューを移植した。BNOコールバック、BNOキュー、UART送信の各件数・損失をBENCHイベントとして残す。まずBOAT24（同一条件60秒）で両ノードの経路を確認し、合格後にBOAT23（180秒）を実施する。
