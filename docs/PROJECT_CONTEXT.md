...
現在進めている実装対象は、統合の第一段階である**制御側XIAO ESP32S3**である。対象プロジェクトは `xiao-boat-control-integration`。BNO08X、VL53L5CX、INA226、PCA9685、手元のサーボ1個、VESC UART、制御側の安全状態機械、通信側へ送るUARTテレメトリー、USB診断を同時に成立させる。
...
GNSS、比較BNO、双方向コマンド、時刻同期、TXTログ、停止サマリー、解析ツールは後続の拡張である。現時点で通信・記録側全体を完成済みまたは主な編集対象として扱わない。
